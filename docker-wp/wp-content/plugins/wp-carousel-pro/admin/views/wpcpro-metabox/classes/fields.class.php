 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_WPCP_Fields' ) ) {
	abstract class SP_WPCP_Fields extends SP_WPCP_Abstract {
		...
		public function field_data( $type = '' ) {
			...
			switch ( $type ) {
				...
				case 'all_posts':
					global $post;
					$upload_data       = get_post_meta( $post->ID, 'sp_wpcp_upload_options', true );
					$sp_selected_posts = isset( $upload_data['wpcp_specific_posts'] ) ? $upload_data['wpcp_specific_posts'] : '' ;
					if ( isset( $sp_selected_posts ) && ! empty( $sp_selected_posts ) ) {
						$wpcp_get_specific = array(
							'post_type' => $upload_data['wpcp_post_type'],
						);
						$query_args = array_merge( $query_args, $wpcp_get_specific );
