	...
	if ( $product_query->have_posts() ) {
		while ( $product_query->have_posts() ) :
			...
			if ( has_post_thumbnail() && $show_product_image ) {
				...
				if ( $image_src ) {
					if ( 'false' !== $lazy_load_image && 'ticker' !== $carousel_mode ) {
						...
						$wpcp_product_thumb = sprintf( '<img class="wpcp-product-image skip-lazy" src="%1$s"%2$s alt="%3$s" width="%4$s" height="%5$s">', $image_src, $image_title_attr, $product_thumb_alt_text, $image_width_attr, $image_height_attr );
					}
			...
			if ( $wpcp_name_chars_limit ) {
				$wpcp_product_name = sprintf( '<h2 class="wpcp-product-title"><a href="%1$s">%2$s</a></h2>', get_the_permalink(), wp_html_excerpt( get_the_title(), $wpcp_name_chars_limit, '...' ) );
			} else {
		...
		$outline .= apply_filters( 'wpcp_product_no_content_found_text', '<h2 class="sp-not-found-any-post" >' . esc_html__( 'No products found', 'wp-carousel-pro' ) . '</h2>' );
	}
	echo '</div>
