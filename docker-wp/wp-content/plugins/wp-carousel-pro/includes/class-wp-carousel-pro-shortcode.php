...
class WP_Carousel_Pro_Shortcode {
	...
	public function sp_wp_carousel_shortcode( $attributes ) {
		...
		if ( ! in_array( $license_status, $if_license_status ) && 1 === version_compare( $first_version, '3.2.3' ) ) {
			if ( current_user_can( 'manage_options' ) ) {
				$activation_notice = sprintf(
					'<div class="wp-carousel-pro-license-notice" style="background: #ffebee;color: #444;padding: 18px 16px;border: 1px solid #d0919f;border-radius: 4px;font-size: 18px;line-height: 28px;">Please <strong><a href="%1$s">activate</a></strong> the license key to get the output of the <strong>WordPress Carousel Pro</strong> plugin.</div>',
		...
		// Carousel Row.
		$carousel_row = isset( $shortcode_data['wpcp_carousel_row'] ) ? $shortcode_data['wpcp_carousel_row'] : '';
		if ( 'standard' === $carousel_mode && 'horizontal' === $carousel_orientation ) {
			$row_lg_desktop = isset( $carousel_row['lg_desktop'] ) ? $carousel_row['lg_desktop'] : 1;
			$row_desktop    = isset( $carousel_row['desktop'] ) ? $carousel_row['desktop'] : 1;
			$row_laptop     = isset( $carousel_row['laptop'] ) ? $carousel_row['laptop'] : 1;
			$row_tablet     = isset( $carousel_row['tablet'] ) ? $carousel_row['tablet'] : 1;
			$row_mobile     = isset( $carousel_row['mobile'] ) ? $carousel_row['mobile'] : 1;
		} else {
			$row_lg_desktop = 1;
			$row_desktop    = 1;
			$row_laptop     = 1;
		...
		// Magnific Script.
		if ( $light_box || 'video-carousel' === $carousel_type || $image_link_show === 'l_box' ) {
		...
		if ( 'ticker' === $carousel_mode ) {
			...
			if ( 'true' == $vertical ) {
				$wpcp_mode         = 'vertical';
				$carousel_classes .= ' vertical';
			} else {
				$wpcp_mode = 'horizontal';
			}
			$wpcp_bx_config = ' data-mode="' . $wpcp_mode . '" data-max-slides="' . $max_column . '" data-min-slides="' . $min_column . '" data-hover-pause="' . $pause_on_hover . '" data-speed="' . $ticker_speed . '" data-slide-width="' . $slide_width . '" data-slide-margin="' . $slide_margin . '" data-direction="' . $auto_direction . '"';
		} else {
			...
			$wpcp_slick_options = 'data-slick=\'{"vertical":' . $vertical . ', "accessibility":true, "centerMode":' . $center_mode . ', "centerPadding": "' . $center_padding . 'px", "swipeToSlide":' . $swipetoslide . ', "adaptiveHeight":' . $adaptive_height . ', "arrows":' . $arrows . ', "autoplay":' . $auto_play . ', "autoplaySpeed":' . $autoplay_speed . ', "dots":' . $dots . ', "infinite":' . $infinite . ', "speed":' . $speed . ', "pauseOnHover":' . $pause_on_hover . ', "slidesToScroll":' . $slide_to_scroll_lg_desktop . ', "slidesToShow":' . $column_lg_desktop . ', "responsive": [{ "breakpoint":' . $desktop_size . ', "settings": { "slidesToShow":' . $column_desktop . ',"rows":' . $row_desktop . ', "slidesToScroll":' . $slide_to_scroll_desktop . ', "centerPadding": "' . $center_padding_desktop . 'px"} }, { "breakpoint":' . $laptop_size . ', "settings":{ "slidesToShow":' . $column_laptop . ', "rows":' . $row_laptop . ', "slidesToScroll":' . $slide_to_scroll_laptop . ', "centerPadding": "' . $center_padding_laptop . 'px" } }, { "breakpoint":' . $tablet_size . ', "settings": { "slidesToShow":' . $column_tablet . ', "slidesToScroll":' . $slide_to_scroll_tablet . ',"rows":' . $row_tablet . ', "centerPadding": "' . $center_padding_tablet . 'px" } }, {"breakpoint":' . $mobile_size . ', "settings":{ "slidesToShow":' . $column_mobile . ', "slidesToScroll":' . $slide_to_scroll_mobile . ',"rows":' . $row_mobile . ', "arrows": ' . $arrows_mobile . ', "dots": ' . $dots_mobile . ', "centerPadding": "' . $center_padding_mobile . 'px" } } ], "rows":' . $row_lg_desktop . ',"rtl":' . $rtl . ', "variableWidth":' . $variable_width . ', "fade":' . $slider_animation . ', "lazyLoad": "' . $lazy_load_image . '", "swipe": ' . $swipe . ', "draggable": ' . $draggable . ' }\' data-arrowtype="' . $nav_icons . '"' . $the_rtl . '';
		}
	 ...
	 * @return void
	 */
	public function fetch_highest_res( $wpcp_video_id ) {
		$resolutions = array( 'maxresdefault', 'hqdefault', 'mqdefault','sddefault', 'default');
		foreach ( $resolutions as $res ) {
			$resolution = apply_filters( 'wp_carousel_youtube_thumb_size', $res );
			$thumb_url  = "https://img.youtube.com/vi/$wpcp_video_id/$resolution.jpg";
		...
		foreach ( $_video_sources as $_video_source ) {
			...
			switch ( $video_type ) {
				case 'youtube':
					...
					$wpcp_video_thumb_url = $this->fetch_highest_res( $wpcp_video_id );
					break;
				case 'vimeo':
