...
foreach ( $sp_urls as $sp_url ) {
	...
	if ( filter_var( $image_src, FILTER_VALIDATE_URL ) && 'image_only' !== $sp_url['video_type'] ) {
		$thumb = sprintf( '<a class="wcp-video" data-fancybox="wpcp_view" data-buttons=["zoom","slideShow","fullScreen","share","download","thumbs","close"] href="%1$s"><img src="%2$s" width="%3$s" height="%4$s"><i class="fa fa-play-circle-o" aria-hidden="true"></i></a>', $sp_url['video_url'], $image_src, $image_width_attr, $image_height_attr );
		...
		if ( 'img_link' === $sp_url['img_click_action'] && ! empty( $image_linking_url ) ) {
			...
			$thumb = sprintf( '<a class="wcp-light-box" href="%1$s" data-fancybox="wpcp_view" data-buttons=["zoom","slideShow","fullScreen","share","download","thumbs","close"] data-lightbox-gallery="group-%3$s">%2$s<i class="fa fa-search-plus" aria-hidden="true"></i></a>', $wpcp_img_full, $thumb, $post_id );     }
	}
