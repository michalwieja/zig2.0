		...
		while ( $post_query->have_posts() ) :
			...
			if ( has_post_thumbnail( $post_query->post->ID ) && $show_slide_image ) {
				...
				if ( 'false' !== $lazy_load_image && 'ticker' !== $carousel_mode ) {
					...
					$post_thumb = sprintf( '<img class="wpcp-post-thumb skip-lazy" src="%1$s"%2$s alt="%3$s" width="%4$s" height="%5$s">', $image_src, $image_title_attr, $image_alt_text, $image_width_attr, $image_height_attr );
				}
			...
			$wpcp_title = sprintf( '<h2 class="wpcp-post-title"><a href="%1$s" target="%2$s">%3$s</a></h2>', apply_filters( 'wpcp_post_title_url', get_the_permalink() ), $link_target, $get_wpcp_title );
			$wpcp_title = apply_filters( 'sp_wpcp_post_title_output', $wpcp_title );

			$wpcp_post_title = ( $show_img_title && ! empty( $get_wpcp_title ) ) ? $wpcp_title : '';
