 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_WPCP_Options' ) ) {
	class SP_WPCP_Options extends SP_WPCP_Abstract {
		...
		public function add_admin_bar_menu( $wp_admin_bar ) {
			...
			if ( ! empty( $this->args['show_bar_menu'] ) && empty( $this->args['menu_hidden'] ) ) {
				...
				$menu_icon = '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 720 566" style="enable-background:new 0 0 720 566;" xml:space="preserve"><style type="text/css">.st0{fill:#1DAEB9;} </style> <g> <g> <polygon class="st0" points="540.8,322 462.3,224.4 509.9,186 588.5,283.7"/> <polygon class="st0" points="510.2,381.4 462.5,343.1 541.1,245.4 588.7,283.8 "/></g><g> <polygon class="st0" points="179.9,322 132.3,283.7 210.8,186 258.5,224.4"/> <polygon class="st0" points="210.6,381.4 132,283.8 179.7,245.4 258.3,343.1"/> </g> </g><g><path class="st0" d="M711.3,556.9H9.4V10.6h701.9V556.9z M88.7,477.6H632V89.9H88.7V477.6z"/></g></svg>';
		...
		public function ajax_save() {
			...
			if ( ! empty( $_POST['data'] ) ) {
				...
				if ( ! empty( $_POST[ 'wpcp_spf_options_nonce'] ) && wp_verify_nonce( $_POST[ 'wpcp_spf_options_nonce'], 'wpcp_spf_options_nonce' ) ) {
		...
		public function set_options() {
			...
			if ( wp_verify_nonce( spf_get_var( 'wpcp_spf_options_nonce' ), 'wpcp_spf_options_nonce' ) ) {
				...
				if ( ! empty( $transient['spf_import_data'] ) ) {
					...
					$this->notice = esc_html__( 'Success. Imported backup options.', 'wp-carousel-pro' );
					...
					$this->notice = esc_html__( 'Default options restored.', 'wp-carousel-pro' );
					...
					$this->notice = esc_html__( 'Default options restored for only this section.', 'wp-carousel-pro' );
				...
				if ( empty( $this->notice ) ) {
					$this->notice = esc_html__( 'Settings saved.', 'wp-carousel-pro' );
				}
			}
		...
		public function add_options_html() {
			...
			wp_nonce_field( 'wpcp_spf_options_nonce', 'wpcp_spf_options_nonce' );
			...
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="spf-expand-all" title="' . esc_html__( 'show all options', 'wp-carousel-pro' ) . '"><i class="fa fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="spf-search"><input type="text" placeholder="' . esc_html__( 'Search option(s)', 'wp-carousel-pro' ) . '" /></div>' : '';

			echo '<div class="spf-buttons">';
			echo '<input type="submit" name="' . $this->unique . '[_nonce][save]" class="button button-primary spf-save' . $ajax_class . '" value="' . esc_html__( 'Save', 'wp-carousel-pro' ) . '" data-save="' . esc_html__( 'Saving...', 'wp-carousel-pro' ) . '">';
			echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="spf_transient[reset_section]" class="button button-secondary spf-reset-section spf-confirm" value="' . esc_html__( 'Reset Section', 'wp-carousel-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'wp-carousel-pro' ) . '">' : '';
			echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="spf_transient[reset]" class="button button-secondary spf-reset-all spf-confirm" value="' . esc_html__( 'Reset All', 'wp-carousel-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'wp-carousel-pro' ) . '">' : '';
			echo '</div>';
			...
			foreach ( $this->pre_sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						SP_WPCP::field( $field, $value, $this->unique, 'options' );
			...
			// 	echo '<div class="spf-buttons">';
			// 	echo '<input type="submit" name="spf_transient[save]" class="button button-primary spf-save' . $ajax_class . '" value="' . esc_html__( 'Save', 'wp-carousel-pro' ) . '" data-save="' . esc_html__( 'Saving...', 'wp-carousel-pro' ) . '">';
			// 	echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="spf_transient[reset_section]" class="button button-secondary spf-reset-section spf-confirm" value="' . esc_html__( 'Reset Section', 'wp-carousel-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'wp-carousel-pro' ) . '">' : '';
			// 	echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="spf_transient[reset]" class="button button-secondary spf-reset-all spf-confirm" value="' . esc_html__( 'Reset All', 'wp-carousel-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'wp-carousel-pro' ) . '">' : '';
			// 	echo '</div>';
