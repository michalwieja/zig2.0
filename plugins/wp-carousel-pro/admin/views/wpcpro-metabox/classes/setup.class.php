...
if ( ! class_exists( 'SP_WPCP' ) ) {
	class SP_WPCP {
		...
		public static function init() {
			...
			add_action( 'after_setup_theme', array( 'SP_WPCP', 'setup' ) );
			add_action( 'init', array( 'SP_WPCP', 'setup' ) );
			add_action( 'switch_theme', array( 'SP_WPCP', 'setup' ) );
			add_action( 'admin_enqueue_scripts', array( 'SP_WPCP', 'add_admin_enqueue_scripts' ), 20 );
			...
			if ( ! empty( self::$args['options'] ) ) {
				foreach ( self::$args['options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SP_WPCP_Options::instance( $key, $params );
			...
			if ( ! empty( self::$args['metaboxes'] ) ) {
				foreach ( self::$args['metaboxes'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SP_WPCP_Metabox::instance( $key, $params );
			...
			if ( ! empty( self::$args['widgets'] ) && class_exists( 'WP_Widget_Factory' ) ) {
				...
				foreach ( self::$args['widgets'] as $key => $value ) {
					if ( ! isset( self::$inited[ $key ] ) ) {
						self::$inited[ $key ] = true;
						$wp_widget_factory->register( SP_WPCP_Widget::instance( $key, $value ) );
					}
				}
			}
		...
		public static function maybe_include_field( $type = '' ) {
			if ( ! class_exists( 'SP_WPCP_Field_' . $type ) && class_exists( 'SP_WPCP_Fields' ) ) {
		...
		public static function add_admin_enqueue_scripts() {
			...
			if ( 'sp_wp_carousel' === $the_current_post_type ) {
				...
				wp_enqueue_style( 'spf', WPCAROUSEL_URL . 'admin/views/wpcpro-metabox/assets/css/spf' . $min . '.css', array(), '1.0.0', 'all' );

				// rtl styles.
				if ( is_rtl() ) {
					wp_enqueue_style( 'spf-rtl', WPCAROUSEL_URL . 'admin/views/wpcpro-metabox/assets/css/spf-rtl' . $min . '.css', array(), '1.0.0', 'all' );
				}

				// framework core scripts.
				wp_enqueue_script( 'spf-plugins', WPCAROUSEL_URL . 'admin/views/wpcpro-metabox/assets/js/spf-plugins' . $min . '.js', array(), '1.0.0', true );
				wp_enqueue_script( 'spf', WPCAROUSEL_URL . 'admin/views/wpcpro-metabox/assets/js/spf' . $min . '.js', array( 'spf-plugins' ), '1.0.0', true );
				...
				wp_localize_script(
					...
					array(
						...
						'i18n'          => array(
							...
							'reset_notification'  => esc_html__( 'Restoring options.', 'wp-carousel-pro' ),
							'import_notification' => esc_html__( 'Importing options.', 'wp-carousel-pro' ),
						),
					)
				);
				...
				if ( ! empty( self::$fields ) ) {
					foreach ( self::$fields as $field ) {
						if ( ! empty( $field['type'] ) ) {
								$classname = 'SP_WPCP_Field_' . $field['type'];
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = sprintf( esc_html__( 'Ooops! This field type (%s) can not be used here, yet.', 'wp-carousel-pro' ), '<strong>' . $field_type . '</strong>' );
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			...
			if ( ! empty( $field_type ) ) {
				...
				$classname = 'SP_WPCP_Field_' . $field_type;

				if ( class_exists( $classname ) ) {
					$instance = new $classname( $field, $value, $unique, $where, $parent );
					$instance->render();
				} else {
					echo '<p>' . esc_html__( 'This field class is not available!', 'wp-carousel-pro' ) . '</p>';
				}
			} else {
					  echo '<p>' . esc_html__( 'This type is not found!', 'wp-carousel-pro' ) . '</p>';
			}
